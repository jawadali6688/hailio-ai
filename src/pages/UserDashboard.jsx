...
const Dashboard = ({ tab }) => {
  ...
  const text_to_speech = async () => {
    try {
      ...
      setLoading(true);
      setError(false);
      const response = await API.post("voice/text_to_speech",
        { text, speed, audioUrl: selectedVoice?.clonedUrl, userId: user._id, apiKey: user?.apiKey },
  ...
  const fetchClonedVoices = async () => {
    try {
      const { data } = await API.post("auth/get_user_cloned", { userId });
    ...
    <section className={`h-screen`}>
      {tab === "text_to_speech" && (
        <div className="flex flex-col lg:flex-row  text-white p-6  gap-7">
          ...
          >
            {error && (
              <p className="text-lg font-bold border bg-orange-600 text-black border-white p-2 rounded-lg">
                Bhai Refresh kr k try kr nahi to Paid Account ly - Bhai Paid
                Plans ly lo, kab tk free chalao gy.
              </p>
            )}
             {!user?.voiceAccess && (
              <p className="text-lg font-bold border bg-orange-800 text-gray-200 border-white p-2 rounded-lg">
              You don't have access to generate voices, please contact admin for access. Thank you!
              </p>
            )}
            <textarea
              value={text}
  ...
  >
    ...
    <p>It may take a while, please wait...</p>
                ...
                <>
                  {user?.voiceAccess &&
                    (history?.length >= user?.allowedVoicesRequest ? (
                      <h1 className="text-white bg-orange-700 w-fit px-2 py-1 rounded-lg">
                        Bs bhai bahut krli voice generate
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 bg-orange-500 px-4 py-2 rounded-lg hover:bg-orange-600 duration-200 cursor-pointer active:bg-orange-700 disabled:bg-gray-200 disabled:hover:bg-gray-100 disabled:text-gray-900 h-full"
                      >
            ...
            <div className="text-right text-orange-400 text-sm">
              {text.length} / {user.oneTimeCharacters}
            </div>
              ...
              >
                <div className="bg-gray-000 p-2 rounded-lg space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      onClick={() => setSelectedVoices(null)}
                  ...
                  {!selectedVoice && voices && (
                    <p className="text-red-600 bg-red-100 rounded-lg px-4">
                      No Voice Selected
                    </p>
                  )}
              ...
              />
                ...
                <div className="hidden">
                  <span className=" flex justify-between items-center">
                    ...
                    <label className="bg-gray-700 px-4 py-1 rounded-md text-white">
                      {pitch.toFixed(1)}
                    </label>
                  </span>
